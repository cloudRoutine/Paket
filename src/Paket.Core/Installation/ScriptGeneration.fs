...
module ScriptGeneration =
    ...
    type PathCombine = PathCombine with
        ...
        static member (?<-) (_,dir:DirectoryInfo,group:GroupName) =  Path.Combine (dir.FullName, group.CompareString)
        ...
        static member (?<-) (_,path:string,group:GroupName) =  Path.Combine (path, group.CompareString)
        ...
        static member (?<-) (_,path:string,package:PackageName) =  Path.Combine (path, package.CompareString)
    ...
    let getScriptFolder (rootFolder: DirectoryInfo) (framework: FrameworkIdentifier) (groupName: GroupName) (folderForDefaultFramework: bool) =
        let group = if groupName = Constants.MainDependencyGroup then String.Empty else groupName.CompareString
          ...
          seq {
            ...
            for group, nuget, _ in lockFile.InstalledPackages do
                if groups = [] || List.exists ((=) group) groups then
                    if not (filterNuget nuget.CompareString) then
                        if group.CompareString = mainGroupKey || group.CompareString = mainGroupName then
                            mainGroupSeen := true

                        let model = lockFile.GetInstalledPackageModel (QualifiedPackageName.FromStrings(Some group.CompareString, nuget.CompareString))
                        ...
                        yield group.CompareString, (libs, syslibs)
    ...
    let generateScriptsForRootFolder
        ...
        let getGroupFile group = 
            ...
            let fileName = (sprintf "%s.group.%s" group.CompareString scriptType.Extension).ToLowerInvariant()
